 ...
 * Update by Dhakan Lal on 25-Apr-2024 DM-890
 * Version 1.1
 */
...
public class ScheduleFragement extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,Bundle savedInstanceState) {
        ...
        if (Utils.isNetworkAvailable(getActivity())) {
            ...
            if(db.scheduleSiteCount()>0){
                response_ticket_list = db.ScheduleSite("", "A");
                if (response_ticket_list.getSite_list()!=null && response_ticket_list.getSite_list().size() > 0) {
                    adapterrr = new AdapterSchedule(getActivity(),response_ticket_list,"S");
        ...
        sites_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> arg0, View arg1, int selectedPos,long arg3) {
                //1.1
                //int alreadyJson = 0;
                db.open();
                String schedule_tab_rights = db.getSubMenuRight("Scheduled", "Preventive");
                if(schedule_tab_rights.contains("A")){
                    if(isUserLocation()){
                        //1.1
                        /*if (db.isAlreadyAutoSaveChk(response_ticket_list.getSite_list().get(selectedPos).getTXN_ID()) == 0) {
                            db.insertAutoSaveChkList(response_ticket_list.getSite_list().get(selectedPos).getTXN_ID(),
                                    "", "", "");
                            alreadyJson = 1;
                        }*/

                        if (getContext().getPackageName().equalsIgnoreCase("tawal.com.sa")) {
                            if (Utils.isNetworkAvailable(getContext())) {
                                callGetApiRespnce(response_ticket_list.getSite_list().get(selectedPos).getSITE_ID(),
                                        response_ticket_list.getSite_list().get(selectedPos).getPARAM_ID(), selectedPos);
                            }
                        }
                    }
                }
    ...
    private void callGetApiRespnce(String site_id, String activtyTypeId, int selectedPos) {
        ...
        Call<String> call = request.getMessgae(site_id,activtyTypeId);
        call.enqueue(new Callback<String>() {
            ...
            public void onResponse(Call<String> call, retrofit2.Response<String> response) {
                ...
                if (response.body().equalsIgnoreCase("Success")) {
                    String s = response_ticket_list.getSite_list().get(selectedPos).getAssetFlag();
                    if (response_ticket_list.getSite_list().get(selectedPos).getAssetFlag()!=null
                       && response_ticket_list.getSite_list().get(selectedPos).getAssetFlag().length()>0
                       && response_ticket_list.getSite_list().get(selectedPos)
                            .getAssetFlag().equalsIgnoreCase("1")) {
                        activity(selectedPos,1);
                    }else {
                        activity(selectedPos,0);
                    }
                    /* if (Utils.isNetworkAvailable(getActivity())) {
                        new getScheduleEquipment(getActivity(),selectedPos).execute();
                    } else {
                        Utils.toast(getActivity(), "17" );
                    }*/
                    //activity(selectedPos,1);
                } else if (response.body() != null) {
                    Utils.toastMsg(getContext(), "" + response.body());
                }
            }
            @Override
    ...
    public void searchData(String flag){
        ...
        response_ticket_list = db.ScheduleSite(cs, flag);
        if (response_ticket_list.getSite_list()!=null && response_ticket_list.getSite_list().size() > 0) {
            adapterrr = new AdapterSchedule(getActivity(),response_ticket_list,"S");
            sites_list.setAdapter(adapterrr);
            txt_no_ticket.setVisibility(View.GONE);
    ...
    public void activity(final int selectedPos,int count) {
        ...
        Intent i;
        if(count>0){
            i = new Intent( getActivity(), EquipmentList.class );
        }else{
            i = new Intent(getActivity(), PMChecklist.class);
        }
        i.putExtra("qrCode", "");
        ...
        if(count>0){
            ...
            i.putExtra("assetId", "0");
            i.putExtra("equipmentId", "0");
            if (Utils.isNetworkAvailable(getActivity())) {
                new GetPMCheckList(getActivity(),i,response_ticket_list.getSite_list().get(selectedPos).getSITE_ID(),
    ...
    public class PMGridTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                ...
                String response = Utils.httpPostRequest1(con,url, nameValuePairs,token);//108
                Gson gson = new Gson();
        ...
        protected void onPostExecute(Void result) {
            if (response_ticket_list == null) {
                ...
                db.insertScheduleList(response_ticket_list);
                db.close();
...
//        SimpleDateFormat sdf = new SimpleDateFormat(myFormat, Locale.US);
        ...
        response_ticket_list = db.ScheduleSite(cs, "D");
        if (response_ticket_list.getSite_list()!=null && response_ticket_list.getSite_list().size() > 0) {
            adapterrr = new AdapterSchedule(getActivity(),response_ticket_list,"S");
            sites_list.setAdapter(adapterrr);
            txt_no_ticket.setVisibility(View.GONE);
        ...
        apply.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                if (All_Operator_Id.length() > 0) {
                    db.open();
                    response_ticket_list = db.ScheduleSite(All_Operator_Id.toString(), "aID");
                    if (response_ticket_list.getSite_list()!=null && response_ticket_list.getSite_list().size() > 0) {
                        adapterrr = new AdapterSchedule(getActivity(),response_ticket_list,"S");
                        sites_list.setAdapter(adapterrr);
                        txt_no_ticket.setVisibility(View.GONE);
    ...
    public void equipmentPopup() {
        Dialog equipment_dialog = new Dialog(getActivity(), R.style.FullHeightDialog);
        equipment_dialog.requestWindowFeature( Window.FEATURE_NO_TITLE);
        equipment_dialog.getWindow().setBackgroundDrawableResource(
                R.color.nevermind_bg_color);
        equipment_dialog.getWindow().setSoftInputMode(
                WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);
        equipment_dialog.setContentView(R.layout.activity_type_popup);

        final Window window_SignIn = equipment_dialog.getWindow();
        window_SignIn.setLayout(WindowManager.LayoutParams.MATCH_PARENT,
                WindowManager.LayoutParams.MATCH_PARENT);
        window_SignIn.clearFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);
        equipment_dialog.show();

        final ListView list_view = (ListView) equipment_dialog.findViewById(R.id.list_view);
        TextView apply = (TextView) equipment_dialog.findViewById(R.id.tv_apply);
        apply.setVisibility(View.GONE);
        TextView tv_header = (TextView) equipment_dialog.findViewById(R.id.tv_header);
        tv_header.setText("Equipment");

        ImageView iv_cancel = (ImageView) equipment_dialog.findViewById(R.id.iv_cancel);
        iv_cancel.setOnClickListener(new View.OnClickListener() {
            public void onClick(View arg0) {
                equipment_dialog.cancel();
            }
        });
        ArrayList<String> equipmentList = new ArrayList<String>();
        equipmentList.add("Generators");
        equipmentList.add("Rectifier");
        equipmentList.add("Battery");
        equipmentList.add("Shelter");
        EquipmentAdapter dataAdapter = new EquipmentAdapter
                (getActivity(),R.layout.custom_equipment, equipmentList);
        list_view.setAdapter(dataAdapter);
       /*apply.setOnClickListener(new View.OnClickListener() {
            @Override
           public void onClick(View arg0) {
                ArrayList<Operator> OperatorList = dataAdapter.countryList;
                equipment_dialog.dismiss();
                adapterrr = new AdapterSchedule(getActivity(),response_ticket_list,"S");
                sites_list.setAdapter(adapterrr);
             }
          });*/
    ...
    private class EquipmentAdapter extends ArrayAdapter<String> {
        private ArrayList<String> list;

        public EquipmentAdapter(Context context, int textViewResourceId,ArrayList<String> list) {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                ...
                convertView = vi.inflate(R.layout.custom_equipment, null);
                holder = new ViewHolder();
                holder.name = (TextView) convertView.findViewById(R.id.name);
                convertView.setTag(holder);
                holder.name.setOnClickListener(new View.OnClickListener() {
                    public void onClick(View v) {
                        Intent i = new Intent( getActivity(), AssetScheduleList.class );
                        i.putExtra("equipment",list.get(position));
                        i.putExtra("equipment",list.get(position));
    ...
    private class GetPMCheckList extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            nameValuePairs.add( new BasicNameValuePair( "checkListDate",response_ticket_list.getSite_list().get(selectedPos).getSCHEDULE_DATE() ) );
        ...
        protected void onPostExecute(Void result) {
            if (PMCheckList == null) {
                ...
                if (PMCheckList.getPMCheckListDetail()!=null && PMCheckList.getPMCheckListDetail().size() > 0) {
                    ...
                    //1.1
                    int alreadyJson = 0;
                    if (db.isAlreadyAutoSaveChk(response_ticket_list.getSite_list().get(selectedPos).getTXN_ID(),"0") == 0) {
                        db.insertAutoSaveChkList(response_ticket_list.getSite_list().get(selectedPos).getTXN_ID(),
                                "", "", "","0");
                        alreadyJson = 1;
                    }

                    db.clearCheckList("655",chklistType,"0");
                    db.insertPMCheckListForm(PMCheckList.getPMCheckListDetail(),"655",
                            alreadyJson,txnId,getActivity(),"0","0");
                    db.close();
                }
            } else {
                Utils.toast(getActivity(),"13");
    ...
    public class getScheduleEquipment extends AsyncTask<Void, Void, Void> {
        ...
        String serUrl  ="";
        int selectedPos;
        public getScheduleEquipment(Context con,final int selectedPos) {
            this.con = con;
            this.selectedPos = selectedPos;
        ...
        protected Void doInBackground(Void... params) {
            try {
                ...
                nameValuePairs.add(new BasicNameValuePair("txnId",response_ticket_list.getSite_list().get(selectedPos).getTXN_ID()));
                nameValuePairs.add(new BasicNameValuePair("equipId","-1"));
                if(moduleUrl.equalsIgnoreCase("0")){
