 ...
 */
public class FragPendingSparePart extends Fragment {
    ...
    int refesh_mode = 0;
    String resume_search_by="",resume_tran_type="",resume_vendor="",resume_site_id;
    public FragPendingSparePart() {
    ...
    public void searchFunction() {
        ...
        resume_search_by = sp_search_by.getSelectedItem().toString().trim();
        resume_tran_type = sp_tran_type.getSelectedItem().toString().trim();
        resume_vendor = sp_vender.getSelectedItem().toString();
        resume_site_id = et_searchby_site.getText().toString().trim();

        et_searchby_site.setText(""+resume_site_id);

        addItemsOnSpinner(sp_search_by, searchDDLlist);
        int resumePos1 = getCategoryPos(resume_search_by,searchDDLlist);
        sp_search_by.setSelection(resumePos1);

        addItemsOnSpinner(sp_vender, vendor_list);
        int resumePos2 = getCategoryPos(resume_vendor,vendor_list);
        sp_vender.setSelection(resumePos2);

        addItemsOnSpinner(sp_tran_type, tranType_list);
        int resumePos3 = getCategoryPos(resume_tran_type,tranType_list);
        sp_tran_type.setSelection(resumePos3);
        ...
        //et_date_from = (TextView) view.findViewById(R.id.et_date_from);
        et_date_from.setBackgroundResource( R.drawable.icon_calender);
        et_date_from.setText(fromDateCal);

        //et_date_till = (TextView) view.findViewById(R.id.et_date_till);
        et_date_till.setBackgroundResource( R.drawable.icon_calender);
...
//            SimpleDateFormat sdf = new SimpleDateFormat(myFormat, Locale.US);
    ...
    public class GetPendingSpareTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                ...
                nameValuePairs.add(new BasicNameValuePair("appType","1")); //(1--PENDING,2-APPROVED,3-REJECTED )
    ...
    public void onResume() {
        ...
        if(fromDateCal.length()==0 || toDateCal.length()==0){
            fromDateCal = "";
            toDateCal = "";
        }
