...
public class FragAlarmHistory extends Fragment {
    View view;
    ResponceTabList responce_list = null;
        ...
        protected Void doInBackground(Void... params) {
            try {
                ...
                if(moduleUrl.equalsIgnoreCase("0")){
                    url=mAppPreferences.getConfigIP()+ WebMethods.url_GetTicketHistory;
                }else{
                ...
                responce_list = new Gson().fromJson(res,ResponceTabList.class);
            } catch (Exception e) {
                e.printStackTrace();
            }
            return null;
    ...
    void SetRemarks() {
        if (responce_list != null && responce_list.getAlarm()!=null && responce_list.getAlarm().size() > 0) {
            lv.setVisibility(View.VISIBLE);
            txt_no_remarks_added.setVisibility(View.GONE);
            lv.setFastScrollEnabled(true);
            lv.setAdapter(new AdapterAssignHis(getActivity(),responce_list,1));
        } else {
